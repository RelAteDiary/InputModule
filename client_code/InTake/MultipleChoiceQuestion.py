...
# TODO P3 refactor MultipleChoiceQuestion and CheckBoxQuestion to have the same parent class
    ...
    def __init__(
        ...
        if has_other_textbox:
            self.other_radio = RadioButton(
                text="other",
                value="other",
                selected=selected is not None and selected.startswith("other-"),
            )
            self.other_radio.add_event_handler("select", self._show_other_textbox)
            self.panel.add_component(self.other_radio)
            self.other_text = TextBox(
                visible=selected is not None and selected.startswith("other-"),
                text=selected[len("other-") :]
                if selected is not None and selected.startswith("other-")
                else "",
            )
        ...
        if prev_button_link is not None:
            self.prev_button = Button(appearance="outlined", text="previous")
            self.prev_button.add_event_handler("click", self.update_question_answer)
        ...
        if next_button_link is not None:
            self.next_button = Button(appearance="filled", text="next")
            self.next_button.add_event_handler("click", self.update_question_answer)
